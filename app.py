...
def display_games_grid(jeux_filtres: pd.DataFrame, cols_per_row: int = 3):
    ...
    for idx, jeu in enumerate(jeux_filtres.itertuples()):
        with cols[idx % cols_per_row]:
            with st.container(border=True):  # Conteneur avec bordure
                ...
                if hasattr(jeu, 'Boite_de_jeu') and pd.notna(jeu.Boite_de_jeu) and str(jeu.Boite_de_jeu).startswith(('http://', 'https://')):
    ...
    # Mécanisme sur toute la largeur
                ...
                with col2:
                    if hasattr(jeu, 'Règles') and pd.notna(jeu.Règles) and str(jeu.Règles).startswith(('http://', 'https://')) and str(jeu.Règles).lower().endswith('.pdf'):
